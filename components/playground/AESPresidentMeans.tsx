      ...
      <div className="rounded border border-slate-800 bg-slate-900/30 p-4">
        ...
        {liberal && conservative && (
          <div className="mt-3 space-y-3 text-sm text-slate-400">
            <MeanEndpoint label="Most liberal mean" president={liberal} />
            <MeanEndpoint label="Most conservative mean" president={conservative} />
          </div>
        )}
        <div className="mt-4 border-t border-slate-800 pt-4 text-xs leading-relaxed text-slate-500">
          Each row shows one president&apos;s directive-level mean AES score.
          This panel starts in {startYear}, so it begins with Truman.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="text-slate-200">
        {president.short} ({formatScore(president.mean_all)})
      </div>
    </div>
  );
}
...
function PresidentMeanRow({
  ...
  const position = scorePosition(president.mean_all, minMean, maxMean);
  return (
